...
import { Crown, BookOpen, Lock, X, HelpCircle, FileText, Printer, Star, FileJson, CheckCircle } from 'lucide-react';
        ...
        <div className="bg-white w-full max-w-sm rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden relative">
            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    {/* AI NOTES (formerly Free) */}
                    <button 
                        onClick={() => onSelect('NOTES_HTML_FREE')}
                        className="bg-white border-2 border-slate-100 hover:border-teal-200 hover:bg-teal-50 rounded-xl p-3 flex flex-col items-center gap-2 relative group transition-all"
                    >
                        {/* GREEN BADGE */}
                        <div className="absolute -top-2 -right-2 bg-teal-500 text-white p-1 rounded-full shadow-sm">
                        ...
                        <div className="text-center">
                            <p className="font-bold text-xs text-slate-700">AI Notes</p>
                            <p className="text-[9px] text-teal-600 font-bold bg-teal-100 px-2 py-0.5 rounded-full mt-1 inline-block">UNLOCKED</p>
                        </div>
                    </button>

                    {/* ULTRA PDF (formerly Premium) */}
                    <button 
                        onClick={() => canAccess(5, 'NOTES_HTML_PREMIUM') && onSelect('NOTES_HTML_PREMIUM')}
                        className={`border-2 rounded-xl p-3 flex flex-col items-center gap-2 relative group transition-all ${
                            canAccess(5, 'NOTES_HTML_PREMIUM') ? 'bg-white border-slate-100 hover:border-indigo-200 hover:bg-indigo-50' : 'bg-slate-50 border-slate-200 opacity-70'
                        }`}
                    >
                        {/* GOLD BADGE */}
                        <div className="absolute -top-2 -right-2 bg-indigo-500 text-white p-1 rounded-full shadow-sm border-2 border-white">
                        ...
                        <div className="text-center">
                            <p className="font-bold text-xs text-slate-700">Ultra PDF</p>
                            <p className="text-[9px] text-indigo-700 font-bold bg-indigo-100 px-2 py-0.5 rounded-full mt-1 inline-block">
