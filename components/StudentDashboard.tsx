...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate }) => {
  ...
  const handleSessionTopicClick = async (topicName: string, subjectName: string) => {
      ...
      // 2. Fetch Chapters (Robust Search Strategy)
      ...
      try {
          ...
          const searchTopic = cleanName(topicName);
          if (searchTopic.includes(':')) {
             // Handle "Physics: Motion" -> "Motion"
             // But keep original too? No, usually topic is the chapter name.
          }
          
          // Strategy A: Try the mapped targetSubject first
          if (targetSubject) {
             const chapters = await fetchChapters(user.board || 'CBSE', user.classLevel || '10', user.stream || 'Science', targetSubject, 'English');
             const match = chapters.find(ch => {
                 const cTitle = cleanName(ch.title);
                 return cTitle.includes(searchTopic) || searchTopic.includes(cTitle);
             });
             if (match) {
                 foundChapter = match;
                 foundSubject = targetSubject;
                 relatedChapters = chapters;
             }
          }

          // Strategy B: If not found, search ALL subjects (BSEB Fix & General Robustness)
          if (!foundChapter) {
              // We prioritize subjects that might be related (Science/SST)
              // But iterate all just in case.
              for (const subj of allSubjects) {
                  if (subj.id === targetSubject?.id) continue; // Already checked
                  
                  // Optimization: Skip obviously unrelated subjects? 
                  // E.g. if topic is "French Revolution", skip Math.
                  // But checking this requires knowledge. Safe to just check all for now (list is small ~5-10).
                  
                  try {
                      const chapters = await fetchChapters(user.board || 'CBSE', user.classLevel || '10', user.stream || 'Science', subj, 'English');
                      const match = chapters.find(ch => {
                        const cTitle = cleanName(ch.title);
                        return cTitle.includes(searchTopic) || searchTopic.includes(cTitle);
                      });
          ...
          if (foundChapter && foundSubject) {
              // OPEN SELECTION MODAL
              setIsLoadingContent(false); 
              setSeasonContentSelection({ chapter: foundChapter, subject: foundSubject });
  ...
  const renderContentSection = (type: 'VIDEO' | 'PDF' | 'MCQ' | 'AI_NOTES' | 'ULTRA_PDF') => {
      ...
      if (contentViewStep === 'PLAYER' && selectedChapter && selectedSubject) {
          ...
          } else if (type === 'PDF' || type === 'AI_NOTES' || type === 'ULTRA_PDF') {
                      ...
                      <div className="grid grid-cols-1 gap-4">
                          ...
                          <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                              <h3 className="font-bold text-indigo-900 flex items-center gap-2 mb-3">
                                  <Crown size={20} className="text-indigo-600" /> Ultra PDF
                              </h3>
                              <div className="grid grid-cols-2 gap-2">
                                  {getSubjectsList(user.classLevel || '10', user.stream || null).map(s => (
                                      <button key={s.id} onClick={() => { onTabChange('ULTRA_PDF' as any); handleContentSubjectSelect(s); }} className="bg-white p-2.5 rounded-xl text-xs font-bold text-slate-700 shadow-sm border border-indigo-100 text-left hover:bg-indigo-50 transition-colors">
                                          {s.name}
                                      </button>
                                  ))}
                              </div>
                          ...
                          <div className="bg-red-50 p-4 rounded-2xl border border-red-100">
                              <h3 className="font-bold text-red-900 flex items-center gap-2 mb-3">
                                  <Youtube size={20} className="text-red-600" /> Video Lectures
                              </h3>
                              <div className="grid grid-cols-2 gap-2">
                                  {getSubjectsList(user.classLevel || '10', user.stream || null).map(s => (
                                      <button key={s.id} onClick={() => { onTabChange('VIDEO'); handleContentSubjectSelect(s); }} className="bg-white p-2.5 rounded-xl text-xs font-bold text-slate-700 shadow-sm border border-red-100 text-left hover:bg-red-50 transition-colors">
                                          {s.name}
                                      </button>
                                  ))}
                              </div>
                          ...
                          <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
                              <h3 className="font-bold text-purple-900 flex items-center gap-2 mb-3">
                                  <CheckSquare size={20} className="text-purple-600" /> MCQ Practice
                              </h3>
                              <div className="grid grid-cols-2 gap-2">
                                  {getSubjectsList(user.classLevel || '10', user.stream || null).map(s => (
                                      <button key={s.id} onClick={() => { onTabChange('MCQ'); handleContentSubjectSelect(s); }} className="bg-white p-2.5 rounded-xl text-xs font-bold text-slate-700 shadow-sm border border-purple-100 text-left hover:bg-purple-50 transition-colors">
                                          {s.name}
                                      </button>
                                  ))}
                              </div>
                          ...
                          <div className="bg-teal-50 p-4 rounded-2xl border border-teal-100">
                              <h3 className="font-bold text-teal-900 flex items-center gap-2 mb-3">
                                  <Sparkles size={20} className="text-teal-600" /> AI Notes
                              </h3>
                              <div className="grid grid-cols-2 gap-2">
                                  {getSubjectsList(user.classLevel || '10', user.stream || null).map(s => (
                                      <button key={s.id} onClick={() => { onTabChange('AI_NOTES' as any); handleContentSubjectSelect(s); }} className="bg-white p-2.5 rounded-xl text-xs font-bold text-slate-700 shadow-sm border border-teal-100 text-left hover:bg-teal-50 transition-colors">
                                          {s.name}
                                      </button>
                                  ))}
                              </div>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-lg z-50 pb-safe">
            <div className="flex justify-around items-center h-16">
                ...
                <button onClick={() => { onTabChange('COURSES'); setContentViewStep('SUBJECTS'); }} className={`flex flex-col items-center justify-center w-full h-full ${['COURSES', 'PDF', 'MCQ', 'VIDEO', 'ULTRA_PDF', 'AI_NOTES'].includes(activeTab) ? 'text-blue-600' : 'text-slate-400'}`}>
                    <Book size={24} fill={['COURSES', 'PDF', 'MCQ', 'VIDEO', 'ULTRA_PDF', 'AI_NOTES'].includes(activeTab) ? "currentColor" : "none"} />
